...
## Tasks / Subtasks
- [x] Task 1 (Frontend - AC: 1): Add a "Project" dropdown to the `CreateTaskModal`.
- [x] Task 2 (Frontend - AC: 2): Add a "Project" dropdown to the `TaskDetailsModal`.
- [x] Task 3 (Frontend - AC: 3): Implement a project filter on the Kanban board page.
- [x] Task 4 (Frontend - AC: 4): Display the project name on the `TaskCard` component.
- [x] Task 5 (Backend - AC: 1, 2): Modify the `createTask` and `updateTask` methods in `TaskService` to handle project association.
    - [x] Subtask 5.1: Update the corresponding DTOs to include `projectId`.
